...
source.u_mask(Nx/2, 1) = 1;           % Left boundary
source.u_mask(1, Ny/2) = 1;           % Top boundary

source_freq = 500; % [Hz]
source_mag = 1;
% source.uy = source_mag * sin(2 * pi * source_freq * kgrid.t_array);  % X-component of displacement
source.uy = -source_mag * sin(2 * pi * source_freq * kgrid.t_array);  % X-component of displacement



%%
%% Define a 2D source different directions
clear source
source.u_mask = zeros(Nx, Ny);

% Define source locations
source.u_mask(1, Ny/2) = 1;    % Top boundary source (for y-component)
source.u_mask(Nx/2, 1) = 2;    % Left boundary source (for x-component)

source_freq = 500;  % [Hz]
source_mag = 1;

% Create time series for each source component
time_series = source_mag * sin(2 * pi * source_freq * kgrid.t_array);

% Assign time series to each component based on location
source.ux = zeros(2, length(kgrid.t_array));
source.uy = zeros(2, length(kgrid.t_array));

% Activate only the required component for each source
source.uy(1, :) = time_series;    % Top boundary source (y-component only)
source.ux(2, :) = -time_series;   % Left boundary source (x-component only)
